...
class UserStatusCubit extends Cubit<UserState> {
  ...
  Future<void> login(String email, String pw, UserCubit userCubit) async {
    User? user = await userCubit
        .verifyUser(User(id: 1, email: email, password: pw, name: ''));
    if (user != null) {
      await userStatusRepo.login(email, pw, user);
    }
    loadState(user);
  }
}
